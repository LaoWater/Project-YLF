...
# Import ChatOpenAI instead of ChatGoogleGenerativeAI
...
# Change from GEMINI_API_KEY to OPENAI_API_KEY
...
POST_SUBJECT = "Hello World! Intro post about our company, starting out, vision, etc"

# LLM Model Configuration
# Changed models to GPT-4o
...
# --- LLM Initialization ---
# Removed Google-specific safety settings
# safety_settings = {
...
decision_llm = ChatOpenAI(
    # Use api_key instead of google_api_key
    api_key=OPENAI_API_KEY_VALUE,
    model=DECISION_LLM_MODEL,
    temperature=1,
    # Removed safety_settings as they are Google-specific
)
...
platform_llm = ChatOpenAI(
    ...
    temperature=1.1,  # Slightly higher for more creative platform adaptation
    # Removed safety_settings as they are Google-specific
)
...
async def generate_media_content(prompt: str, media_type: Literal["image", "video"]) -> MediaAsset:
    ...
    # Use a stable hash of the prompt for a semi-deterministic URL in simulation
    import hashlib
    prompt_hash = hashlib.md5(prompt.encode('utf-8')).hexdigest()[:8]
    generated_media_url = f"https://example.com/generated_media_{media_type}_{prompt_hash}.{extension}"
...
-   Tone: Conversational, direct, and encouraging. "{company_sentiment}" adapted for brevity. Yet don't get too corny, keep it real.
-   Length: Max 150 characters (Tweets often work best under 130). If {core_post_text_suggestion} is long, create a concise summary or key takeaway. NO THREADS for this task, aim for a single impactful tweet.
...
-   Media (if Type B/C):
    ...
    -   Type C (Informative): Visually engaging but calm background + captions with key messages from "{subject}". Text overlays should be stylish and readable.
    -   Media Prompt: If generating `platform_media_generation_prompt` for an {media_type_for_prompt}, describe visuals for a fast-paced feed. Aspect ratios square 1:1 or 16:9 work well.
...
async def run_platform_adaptation_agent(inputs: PlatformAgentInput) -> PlatformAgentOutput:
    ...
    # Note: The original code had a check for Type D here, but Type D is not in the Literal union for PostHistoryEntry or Layer2Output.
    # Assuming Type C still implies an image for the prompt, based on the prompt descriptions.
    # If Type C should generate a video prompt, change 'image' to 'video' below.
    # If Type D/E were added, you'd extend this logic.
    # For now, sticking to Type B/C generating image prompts as per prompt text.
...
async def generate_social_media_posts(
    ...
    core_post_text = layer2_result["core_post_text"] # Store this
    ...
    for platform_name in target_platforms:
        platform_agent_input = PlatformAgentInput(
            ...
            company_sentiment=COMPANY_SENTIMENT,
            subject=subject,
            post_type_decision=decided_post_type, # Use the decided type from layer 2
            core_post_text_suggestion=core_post_text, # Use the core text from layer 2
    ...
    # Only attempt media generation if the *Layer 2 Decision* was Type B or C
    if decided_post_type in ["B", "C"]:
        media_type_to_generate: Literal["image", "video"] = "image"
        # If you add Type D/E, update this logic based on the decided_post_type
        # if decided_post_type == "D": media_type_to_generate = "video"
        ...
        for platform_name in target_platforms:
            ...
            # Check if the platform agent *actually* returned a media prompt
            media_prompt = platform_output.get("platform_media_generation_prompt")
            if media_prompt:
                # Only generate media if the platform agent provided a prompt
                media_generation_tasks.append(
                    generate_media_content(media_prompt, media_type_to_generate)
                )
                # Add platform_name to this list ONLY if a prompt was generated
                platform_names_needing_media.append(platform_name)
            # If platform_output is not None but media_prompt is None, it means the platform
            # agent decided against media for this specific platform adaptation, even if Layer 2
            # recommended B or C. This is acceptable behaviour based on the platform agent's prompt.


    generated_media_assets_list: List[Optional[MediaAsset]] = []
    if media_generation_tasks:
        print(f"\n--- Running Media Generation for {len(media_generation_tasks)} assets ---")
        generated_media_assets_list = await asyncio.gather(*media_generation_tasks)
    else:
        print("\n--- No Media Generation needed ---")


    # Map generated media back to the correct platform using the platform_names_needing_media list
    generated_media_map: Dict[str, MediaAsset] = {}
    for i, platform_name_with_media in enumerate(platform_names_needing_media):
         # Ensure we have a corresponding generated asset
        if i < len(generated_media_assets_list) and generated_media_assets_list[i] is not None:
    ...
    for platform_name in target_platforms:
        ...
        # Get the media asset for this specific platform from the map
        current_media_asset: Optional[MediaAsset] = generated_media_map.get(platform_name)

        final_posts.append(FinalGeneratedPost(
            platform=platform_name,
            # Use the original decided type from layer 2 for the final output structure
...
async def main_orchestration():
    sample_posts_history: List[PostHistoryEntry] = [
        ...
        {"post_type": "A", "count": 7, "score": 8},
        {"post_type": "A", "count": 7, "score": 8}
        # Add more history here if needed to test history logic influencing decision
    ]

    target_platforms = ["linkedin", "instagram", "twitter", "facebook"]

    # Example requirements (uncomment to test)
    # example_requirements: Requirements = {
    #     "min_length": 50,
    #     "max_length": 200,
    #     "must_include_keywords": ["home therapy", "Terapie Acasa"]
    # }

    generated_posts_list = await generate_social_media_posts(
        subject=POST_SUBJECT,
        target_platforms=target_platforms,
        requirements=None, # Use example_requirements here to test
    ...
    for i, post in enumerate(generated_posts_list):
        ...
        print(f"Text: {post['text'][:300]}...")  # Print summary (increased chars slightly)
        if post['media_asset']:
...
if __name__ == "__main__":
    ...
    if not OPENAI_API_KEY_VALUE:
        ...
        final_json_output = json.dumps(final_generated_posts, indent=4, ensure_ascii=False) # ensure_ascii=False for proper character output
        print("\n\n--- Final JSON Output ---")
        print(final_json_output)
        output_filename = "social_media_posts_output_openai.json" # Changed output filename
