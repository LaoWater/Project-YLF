...
# Limited at 16.000 HZ frequency #
# ###################################################
...
# Suppress warnings from Hugging Face's transformers library
...
def load_whisper_model(model_id, device, torch_dtype, language="en", model_temperature=0.55):
...
def transcribe_audio_file_in_chunks(file_path, model, processor, device, torch_dtype, chunk_duration=30):
    ...
    for i, chunk in enumerate(audio_chunks):
        from warnings_setup import suppress_stdout
...
def main(audio_recording, model_temperature=0.55):
    print(f"Whisper Large V3-Turbo Local Model: (Temperature {model_temperature})")
    ...
    # Seems to be misfiring with provided temperature? check empty
    model, processor = load_whisper_model(model_id, device, torch_dtype, language=language, model_temperature=model_temperature)

    # Step 3: Provide the path to the local .wav file
    audio_file_path = os.path.join("recordings", audio_recording)
