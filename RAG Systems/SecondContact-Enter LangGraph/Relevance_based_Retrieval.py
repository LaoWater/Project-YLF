...
# Define the LLM models - using a smaller model for memory management to save costs
memory_llm = ChatOpenAI(
    openai_api_key=api_key,
    model="gpt-3.5-turbo-0125",  # Using a smaller model for memory operations
...
response_llm = ChatOpenAI(
    openai_api_key=api_key,
    model="ft:gpt-4o-mini-2024-07-18:personal::AioG35Tv"  # Using your fine-tuned model
)
...
class SmartMemoryManager:
    ...
    def _update_conversation_topics(self) -> None:
        ...
        # Use memory_llm to extract topics
        topic_prompt = ChatPromptTemplate.from_messages([
            ("system", "Extract 3-5 key topics from this conversation segment. "
                       "Return only the topics as a comma-separated list, no explanation."),
            ("human", combined_text)
        ])

        topics_response = topic_prompt | memory_llm
        topics_text = topics_response.invoke({}).content
    ...
    def _create_summary_if_needed(self) -> None:
        """Create a summary of older conversation segments"""
        ...
        # Create summary using memory_llm
        summary_prompt = ChatPromptTemplate.from_messages([
            ("system", "Create a concise summary of this conversation segment that preserves "
                       "key information, decisions, and context. Focus on essential details "
                       "that would be needed for continuing this conversation naturally."),
            ("human", combined_text)
        ])

        summary_response = summary_prompt | memory_llm
        summary_text = summary_response.invoke({}).content
    ...
    def _get_relevant_summary(self, query: str) -> Optional[dict]:
        """Find the most relevant conversation summary"""
        if not self.memory_summaries:
            return None

        # Use the memory LLM to select the most relevant summary
        summaries_text = "\n\n".join([
        ...
        selection_prompt = ChatPromptTemplate.from_messages([
            ("system", "Given a user query and several conversation summaries, return ONLY the number "
                       "of the most relevant summary. Return just the number, nothing else."),
            ("human", f"Query: {query}\n\n{summaries_text}")
        ])

        try:
            selection_response = selection_prompt | memory_llm
            selection = selection_response.invoke({}).content.strip()
...
def build_conversation_graph():
    ...
    # Add checkpoint capability for memory persistence
